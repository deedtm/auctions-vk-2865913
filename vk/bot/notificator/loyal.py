...
async def notificate_loyal_users():
    ...
    for days, users in users.items():
        ...
        peer_ids = [user.user_id for user in users]
        for max_ind in range(0, len(peer_ids), 100):
            user_ids = peer_ids[max_ind : max_ind + 100]
            await send_notification(",".join(map(str, user_ids)), text)
